...
describe("SupplyChain", function () {
    ...
    beforeEach(async function () {
        ...
        supplyChain = await SupplyChain.deploy(accessManager.target);
    ...
    describe("Addition & Update", function () {
        ...
        it("Should pushBatchToProcessed()", async function () {
            ...
            await actorsManager.registerActor(0, farmer.address, VALID_HASH); // farmer
            await actorsManager.registerActor(1, processor.address, VALID_HASH); // processor

            await supplyChain.addHarvestedBatch(0, VALID_HASH);
            await supplyChain.pushBatchToProcessed(0, 0, VALID_HASH);
        ...
        it("Should pushBatchToPackaged()", async function () {
            const batch = {
                batchId: 0,
                state: 2, // Packaged
                isCertified: true,
            ...
            await actorsManager.registerActor(0, farmer.address, VALID_HASH); // farmer
            await actorsManager.registerActor(1, processor.address, VALID_HASH); // processor
            await actorsManager.registerActor(2, packager.address, VALID_HASH); // packager

            await supplyChain.addHarvestedBatch(0, VALID_HASH);
            await supplyChain.pushBatchToProcessed(0, 0, VALID_HASH);
            await supplyChain.pushBatchToPackaged(0, 0, VALID_HASH);
        ...
        it("Should assignBatchToDistributor()", async function () {
            const batch = {
                batchId: 0,
                state: 3, // Distributed
                isCertified: true,
            ...
            await supplyChain.pushBatchToProcessed(0, 0, VALID_HASH);
            await supplyChain.pushBatchToPackaged(0, 0, VALID_HASH);
            await supplyChain.assignBatchToDistributor(0, 0, VALID_HASH);
        ...
        it("Should assignBatchToRetailer()", async function () {
            const batch = {
                batchId: 0,
                state: 4, // Distributed
                isCertified: true,
    ...
    describe("Batch retrieval", function () {
        ...
        it("Should getBatchesHarvested(farmerId)", async function () {
            const batches = await supplyChain.getBatchesHarvested(0);
            expect(batches.length).to.equal(1);
        });

        it("Should getBatchesProcessed(processorId)", async function () {
            const batches = await supplyChain.getBatchesProcessed(0);
            expect(batches.length).to.equal(1);
        });

        it("Should getBatchesPackaged(packagerId)", async function () {
            const batches = await supplyChain.getBatchesPackaged(0);
            expect(batches.length).to.equal(1);
        });
